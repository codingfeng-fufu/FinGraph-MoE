...
from .data import load_sum_v2_splits
from .model import SumV2Model
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        predictions, _ = model(batch.x, batch.node_values, batch.batch, batch.query_attr)
...
@torch.no_grad()
def evaluate(model: SumV2Model, loader: DataLoader, device: torch.device) -> dict[str, float]:
    model.eval()
    total_loss = 0.0
    total_graphs = 0
    predictions_all = []
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the real-data SUM v2 expert.")
    parser.add_argument("--dataset-file", type=str, default="data/realdata_inputs/sum_dataset2.jsonl")
    parser.add_argument("--output-dir", type=str, default="runs/realdata_experts/sum_v2")
...
def run_training(args: argparse.Namespace) -> dict[str, object]:
    ...
    train_graphs, val_graphs, test_graphs, split_summary = load_sum_v2_splits(
        args.dataset_file,
        seed=args.seed,
    ...
    model = SumV2Model(hidden_dim=args.hidden_dim).to(device)
