"""Train PredictV6 hybrid recurrent + in-context peer expert."""
...
from .data import load_predict_v6_splits
from .model import PredictV6Model
...
def _predict(model: PredictV6Model, batch: dict[str, torch.Tensor]) -> torch.Tensor:
    return model(
        batch["history_seq"],
        batch["history_mask"],
        batch["context_seq"],
...
@torch.no_grad()
def evaluate(model: PredictV6Model, loader: DataLoader, device: torch.device) -> dict[str, float]:
    model.eval()
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train PredictV6 hybrid recurrent + in-context expert.")
    parser.add_argument("--dataset-file", default="data/realdata_inputs/prediction_dataset.jsonl")
    parser.add_argument("--output-dir", default="runs/realdata_experts/predict_v6")
    ...
    parser.add_argument("--history-mode", choices=("none", "gru", "lstm"), default="lstm")
    parser.add_argument("--context-mode", choices=("none", "deepsets", "attention"), default="attention")
...
def _compact_summary(results: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "name": Path(results["checkpoint_path"]).parent.name,
        "best_epoch": results["best_epoch"],
        "model_config": results["model_config"],
        "val_loss": val["loss"],
        "test_growth_mse": test["growth_mse"],
        "test_growth_mae": test["growth_mae"],
        "test_growth_r2": test["growth_r2"],
        "test_revenue_mae": test["revenue_mae"],
        "test_revenue_rmse": test["revenue_rmse"],
        "test_revenue_mape": test["revenue_mape"],
        "baseline_revenue_mape": test["baseline_revenue_mape"],
...
def main() -> None:
    ...
    payload = _compact_summary(results) if args.quiet else results
